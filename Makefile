...
# BITS is the pointer width to build/link for. The caller may set it
# explicitly to either 32 or 64 if those are supported by the installed
# toolchain, otherwise it defaults to the host's natural bitness.
#
# NOTE: The program generated by QB64-PE needs to have been for the same
#       bitness used here. See -f:TargetBits on qb64pe.exe
...
ifeq ($(OS),win)
	...
	# For supported architectures, should be i686, x86_64, aarch64
	# -dumpmachine gives a target triple, for what we care about it's the text
	# before the first '-'
	ARCH = $(word 1,$(subst -, ,$(shell $(CC) -dumpmachine)))

	ifndef BITS
		# Check bitness based on detected architectures
...
# These flags tell the toolchain whether to compile for 32bit or 64bit, they
# apply to everything compiled for libqb.
ifeq ($(BITS),32)
	CXXFLAGS += -m32
	CXXLIBS += -m32
	CFLAGS += -m32
