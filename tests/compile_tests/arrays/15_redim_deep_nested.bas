...
ReDim reference(0 To 3) As Integer
ReDim items(1).Groups(0).Nodes(1).Datas(0 To 3)

For idx = 0 To 3
    If items(1).Groups(0).Nodes(1).Datas(idx) <> reference(idx) Then
        Print "FAIL 15_redim_deep_nested.bas: index"; idx; " expected "; reference(idx); " got "; items(1).Groups(0).Nodes(1).Datas(idx)
        System
    End If
Next idx

Print "PASS 15_redim_deep_nested.bas"
System
