...
void sub__displayorder(int32 method1, int32 method2, int32 method3,
                       int32 method4) {
...
void prepare_environment_2d() { // called prior to rendering 2D content
    ...
    if (environment_2d__screen_width == environment__window_width &&
        ...
        window_ratio = (float)environment__window_width /
                       (float)environment__window_height;
        screen_ratio = (float)environment_2d__screen_width /
                       (float)environment_2d__screen_height;
        if (can_scale == 0) {
        ...
        if (need_square_pixels == 0 || (window_ratio == screen_ratio)) {
            ...
            static int32 x1, y1, x2, y2, z, x_limit, y_limit, x_offset,
                y_offset;
            ...
            if (window_ratio > screen_ratio) {
                // pad sides
                z = (float)environment__window_height *
                    screen_ratio; // new width
                ...
                // pad top/bottom
                z = (float)environment__window_width /
                    screen_ratio; // new height
                y1 = environment__window_height / 2 - z / 2;
                y2 = y1 + z - 1;
                environment_2d__letterbox =
                    2; // horizontal black stripes required
                // y_offset=-y1;
    ...
    cant_scale:
        environment_2d__screen_scaled_width =
            environment_2d__screen_x2 - environment_2d__screen_x1 + 1;
        environment_2d__screen_scaled_height =
            environment_2d__screen_y2 - environment_2d__screen_y1 + 1;
        environment_2d__screen_x_scale =
            (float)environment_2d__screen_scaled_width /
            (float)environment_2d__screen_width;
        environment_2d__screen_y_scale =
            (float)environment_2d__screen_scaled_height /
            (float)environment_2d__screen_height;
    }
...
int32 environment_2d__get_window_x1_coord(int32 x) {
    return qbr_float_to_long(((float)x) * environment_2d__screen_x_scale) +
           environment_2d__screen_x1;
}
int32 environment_2d__get_window_y1_coord(int32 y) {
    return qbr_float_to_long((float)y * environment_2d__screen_y_scale) +
           environment_2d__screen_y1;
}
int32 environment_2d__get_window_x2_coord(int32 x) {
    return qbr_float_to_long(
               ((float)x + 1.0f) * environment_2d__screen_x_scale - 1.0f) +
           environment_2d__screen_x1;
}

int32 environment_2d__get_window_y2_coord(int32 y) {
    return qbr_float_to_long(
               ((float)y + 1.0f) * environment_2d__screen_y_scale - 1.0f) +
           environment_2d__screen_y1;
}
...
environment_2d__window_rect_struct *
environment_2d__screen_to_window_rect(int32 x1, int32 y1, int32 x2, int32 y2) {
    tmp_rect.x1 =
        qbr_float_to_long(((float)x1) * environment_2d__screen_x_scale) +
        environment_2d__screen_x1;
    tmp_rect.y1 =
        qbr_float_to_long(((float)y1) * environment_2d__screen_y_scale) +
        environment_2d__screen_y1;
    static int32 w, h;
    w = abs(x2 - x1) + 1;
    h = abs(y2 - y1) + 1;
        ...
        if (hardware_buffer_vertices_count == hardware_buffer_texcoords_count) {
            ...
            glDrawArrays(GL_TRIANGLES, 0,
                         hardware_buffer_vertices_count /
                             2); // start index, number of indexes
        } else {
            glVertexPointer(
                3, GL_FLOAT, 3 * sizeof(GL_FLOAT),
                hardware_buffer_vertices); // http://www.opengl.org/sdk/docs/man2/xhtml/glVertexPointer.xml
            glTexCoordPointer(
                2, GL_FLOAT, 2 * sizeof(GL_FLOAT),
                hardware_buffer_texcoords); // http://www.opengl.org/sdk/docs/man2/xhtml/glTexCoordPointer.xml
            glDrawArrays(GL_TRIANGLES, 0,
                         hardware_buffer_vertices_count /
                             3); // start index, number of indexes
        }
...
void set_smooth(int32 new_mode_shrunk, int32 new_mode_stretched) {
    ...
    if (new_mode_shrunk == current_mode_shrunk &&
        new_mode_stretched == current_mode_stretched)
        return;
    ...
    if (new_mode == ALPHA_MODE__BLEND) {
        ...
        if (framebufferobjects_supported) {
            ...
            glBlendFuncSeparateEXT(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, GL_ONE,
                                   GL_ONE);
#else
            glBlendFuncSeparateOES(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, GL_ONE,
                                   GL_ONE);
#endif
        } else {
...
void set_view(int32 new_mode) { // set view can only be called after the correct
    ...
    if (new_mode == VIEW_MODE__2D) {
        ...
        if (render_state.dest_handle == 0) {
                ...
                if (consoleStream) {
                    ...
                    while (!feof(consoleStream)) {
                        if (fgets(buffer, 128, consoleStream) != NULL) {
                            ...
                            if (!b_isRetina)
                                b_isRetina =
                                    (szBuffer.rfind("Retina") != ULONG_MAX);
                            if (!b_is5k)
            ...
            hardware_img = (hardware_img_struct *)list_get(
                hardware_img_handles, render_state.dest_handle);
            glMatrixMode(GL_PROJECTION);
            glLoadIdentity();
    ...
    if (new_mode == VIEW_MODE__3D) {
        ...
        if (render_state.dest_handle == 0) {
            ...
            float fov;
            if (environment_2d__screen_scaled_width >
                environment_2d__screen_scaled_height) {
                fov = 90.0f * ((float)environment__window_width /
                               (float)environment_2d__screen_scaled_width);
                // convert fov from horizontal to vertical
                fov = fov * ((float)dst_h / (float)dst_w);
            } else {
                fov = 90.0f * ((float)environment__window_height /
                               (float)environment_2d__screen_scaled_height);
            }
...
void set_render_source(int32 new_handle) {
    ...
    hardware_img_struct *hardware_img;
    hardware_img =
        (hardware_img_struct *)list_get(hardware_img_handles, new_handle);
    ...
    if (new_handle == 0) {
        ...
        if (hardware_img->dest_context_handle == 0) {
            ...
            glBindFramebufferEXT(GL_FRAMEBUFFER,
                                 hardware_img->dest_context_handle);
        }
...
void hardware_img_put(int32 dst_x1, int32 dst_y1, int32 dst_x2, int32 dst_y2,
                      int32 src_img, int32 dst_img, int32 src_x1, int32 src_y1,
                      int32 src_x2, int32 src_y2, int32 use_alpha,
                      int32 smooth) {
    ...
    if (dst_img) {
        ...
        rect = environment_2d__screen_to_window_rect(dst_x1, dst_y1, dst_x2,
                                                     dst_y2);
        dst_x1 = rect->x1;
        dst_y1 = rect->y1;
        dst_x2 = rect->x2;
    ...
    src_hardware_img =
        (hardware_img_struct *)list_get(hardware_img_handles, src_img);
    ...
    if ((hardware_buffer_vertices_count + 18) > hardware_buffer_vertices_max) {
        ...
        hardware_buffer_vertices =
            (float *)realloc(hardware_buffer_vertices,
                             hardware_buffer_vertices_max * sizeof(float));
    }
    if ((hardware_buffer_texcoords_count + 12) >
        hardware_buffer_texcoords_max) {
        hardware_buffer_texcoords_max = hardware_buffer_texcoords_max * 2 + 12;
        hardware_buffer_texcoords =
            (float *)realloc(hardware_buffer_texcoords,
...
void hardware_img_tri2d(float dst_x1, float dst_y1, float dst_x2, float dst_y2,
                        float dst_x3, float dst_y3, int32 src_img,
                        int32 dst_img, float src_x1, float src_y1, float src_x2,
                        float src_y2, float src_x3, float src_y3,
                        int32 use_alpha, int32 smooth) {
    ...
    if (dst_img) {
        static hardware_img_struct *dst_hardware_img;
        dst_hardware_img =
            (hardware_img_struct *)list_get(hardware_img_handles, dst_img);
        ...
        static int32 basex;
        basex =
            qbr_float_to_long(((float)(rx1)) * environment_2d__screen_x_scale +
                              (float)environment_2d__screen_x1);
        dst_x1 = basex + qbr_float_to_long(((float)(dst_x1 - rx1)) *
                                           environment_2d__screen_x_scale * xr);
        dst_x2 = basex + qbr_float_to_long(((float)(dst_x2 - rx1)) *
                                           environment_2d__screen_x_scale * xr);
        dst_x3 = basex + qbr_float_to_long(((float)(dst_x3 - rx1)) *
                                           environment_2d__screen_x_scale * xr);
        static int32 basey;
        basey =
            qbr_float_to_long(((float)(ry1)) * environment_2d__screen_y_scale +
                              (float)environment_2d__screen_y1);
        dst_y1 = basey + qbr_float_to_long(((float)(dst_y1 - ry1)) *
                                           environment_2d__screen_y_scale * yr);
        dst_y2 = basey + qbr_float_to_long(((float)(dst_y2 - ry1)) *
                                           environment_2d__screen_y_scale * yr);
        dst_y3 = basey + qbr_float_to_long(((float)(dst_y3 - ry1)) *
                                           environment_2d__screen_y_scale * yr);
    }
...
void clear_depthbuffer(int32 dst_img) {
    ...
    set_render_dest(dst_img);
    if (dst_img > 0) {
        hardware_img_requires_depthbuffer(
            (hardware_img_struct *)list_get(hardware_img_handles, dst_img));
    }
    glClear(GL_DEPTH_BUFFER_BIT);
}

void hardware_img_tri3d(float dst_x1, float dst_y1, float dst_z1, float dst_x2,
                        float dst_y2, float dst_z2, float dst_x3, float dst_y3,
                        float dst_z3, int32 src_img, int32 dst_img,
                        float src_x1, float src_y1, float src_x2, float src_y2,
                        float src_x3, float src_y3, int32 use_alpha,
...
void GLUT_DISPLAY_REQUEST() {
    ...
    highest_order = 0;
    if (last_i != -1)
        highest_order = display_frame[last_i]
                            .order; // avoid any frames below the current one
    for (i2 = 0; i2 <= 2; i2++) {
        if (display_frame[i2].state == DISPLAY_FRAME_STATE__READY &&
            display_frame[i2].order > highest_order) {
    ...
    if (i != last_i) {
        for (i2 = 0; i2 <= 2; i2++) {
            if (display_frame[i2].order < display_frame[i].order &&
                (display_frame[i2].state == DISPLAY_FRAME_STATE__DISPLAYING ||
                 display_frame[i2].state == DISPLAY_FRAME_STATE__READY))
    ...
    if (last_rendered_hardware_display_frame_order ==
        last_hardware_display_frame_order) {
        if (i == last_i) {
            if (full_screen_set == -1) {    // no pending full-screen changes
                if (os_resize_event == 0) { // no resize events
                    ...
                    if (first_command_prev_order) {
                        ...
                        // (if any)
                        last_hardware_command_rendered =
                            first_command_prev_order;
                    }
                }
            }
        }
    ...
    if ((full_screen == 0) &&
        (full_screen_set ==
         -1)) { // not in (or attempting to enter) full screen

        display_required_x = display_frame[i].w;
        display_required_y = display_frame[i].h;
        static int32 framesize_changed;
        framesize_changed = 0;
        if ((display_required_x != resize_snapback_x) ||
            (display_required_y != resize_snapback_y))
            framesize_changed = 1;

        resize_auto_ideal_aspect =
            (float)display_frame[i].w / (float)display_frame[i].h;
        ...
        if (resize_auto) {
            ...
            if ((ar != resize_auto_accept_aspect) &&
                (ar != resize_auto_ideal_aspect)) {
                // set new size
                static int32 x, y;
                ...
                if ((display_y_prev != display_y) &&
                    ...
                    if (abs(display_y_prev - display_y) <
                        abs(display_x_prev - display_x)) {
                        x = display_x;
                        y = (float)x / resize_auto_ideal_aspect;
                    } else {
                        y = display_y;
        ...
        if ((display_required_x != display_x) ||
            (display_required_y != display_y)) {
    ...
    */
    ...
    for (level = 0; level <= 5; level++) {
        ...
        if (level == 0) {
            ...
            if (level == displayorder_glrender) {
                ...
                if (close_program || dont_call_sub_gl || suspend_program ||
                    stop_program)
                    goto abort_gl;
            ...
            if (level == displayorder_screen) { // defaults to 1
                ...
                if (i != last_i || software_screen_hardware_frame == 0) {
                    software_screen_hardware_frame =
                        new_hardware_img(display_frame[i].w, display_frame[i].h,
                                         display_frame[i].bgra, NULL);
                }

                static hardware_img_struct *f1;
                f1 = (hardware_img_struct *)list_get(
                    hardware_img_handles, software_screen_hardware_frame);
                ...
                hardware_img_put(0, 0, environment_2d__screen_width - 1,
                                 environment_2d__screen_height - 1,
                                 software_screen_hardware_frame, 0, 0, 0,
                                 f1->w - 1, f1->h - 1, use_alpha,
                                 environment_2d__screen_smooth);
                hardware_buffer_flush();

            } // level==displayorder_screen

            if (level == displayorder_hardware ||
                level == displayorder_hardware1) {
                ...
                if (first_hardware_layer_rendered == 0) {
                    ...
                    if (first_hardware_command) {
                        ...
                        if (last_hardware_command_rendered) {
                            ...
                            if (rerender_prev_hardware_frame) {
                                ...
                                hardware_graphics_command_struct *last_hgc =
                                    (hardware_graphics_command_struct *)
                                        list_get(
                        ...
                        // frame's order value
                        while (command) {
                            hardware_graphics_command_struct *hgc =
                                (hardware_graphics_command_struct *)list_get(
                                    hardware_graphics_command_handles, command);
                            if (hgc->order < order) {

                                if (hgc->command ==
                                    HARDWARE_GRAPHICS_COMMAND__FREEIMAGE) {
                                    free_hardware_img(hgc->src_img,
                                                      847002 + caller_flag);
                                }

                                if (hgc->command ==
                                    HARDWARE_GRAPHICS_COMMAND__PUTIMAGE) {
                                    if (hgc->dst_img >
                                        0) { // note: rendering to the old
                                             // default surface is pointless, but
                                             // renders onto maintained hardware
                                             // images are still required
                                        hardware_img_put(
                                            hgc->dst_x1, hgc->dst_y1,
                                            hgc->dst_x2, hgc->dst_y2,
                                            hgc->src_img, hgc->dst_img,
                                            hgc->src_x1, hgc->src_y1,
                                            hgc->src_x2, hgc->src_y2,
                                            hgc->use_alpha, hgc->smooth);
                                    }
                                }

                                if (hgc->command ==
                                    HARDWARE_GRAPHICS_COMMAND__MAPTRIANGLE) {
                                    if (hgc->dst_img >
                                        0) { // note: rendering to the old
                                             // default surface is pointless, but
                                             // renders onto maintained hardware
                                        ...
                                        hardware_img_tri2d(
                                            hgc->dst_x1, hgc->dst_y1,
                                            hgc->dst_x2, hgc->dst_y2,
                                            hgc->dst_x3, hgc->dst_y3,
                                            hgc->src_img, hgc->dst_img,
                                            hgc->src_x1, hgc->src_y1,
                                            hgc->src_x2, hgc->src_y2,
                                            hgc->src_x3, hgc->src_y3,
                                            hgc->use_alpha, hgc->smooth);
                                    }
                                }

                                if (hgc->command ==
                                    HARDWARE_GRAPHICS_COMMAND__MAPTRIANGLE3D) {
                                    if (hgc->dst_img >
                                        0) { // note: rendering to the old
                                             // default surface is pointless, but
                                             // renders onto maintained hardware
                                        ...
                                        hardware_img_tri3d(
                                            hgc->dst_x1, hgc->dst_y1,
                                            hgc->dst_z1, hgc->dst_x2,
                                            hgc->dst_y2, hgc->dst_z2,
                                            hgc->dst_x3, hgc->dst_y3,
                                            hgc->dst_z3, hgc->src_img,
                                            hgc->dst_img, hgc->src_x1,
                                            hgc->src_y1, hgc->src_x2,
                                            hgc->src_y2, hgc->src_x3,
                                            hgc->src_y3, hgc->use_alpha,
                                            hgc->smooth, hgc->cull_mode,
                                            hgc->depthbuffer_mode);
                                    }
                                }

                                if (hgc->command ==
                                    HARDWARE_GRAPHICS_COMMAND__CLEAR_DEPTHBUFFER) {
                                    if (hgc->dst_img >
                                        0) { // note: rendering to the old
                                             // default surface is pointless, but
                                             // renders onto maintained hardware
                ...
                while (command) {
                    ...
                    if (hgc->order == order) {
                        ...
                        if (hgc->command ==
                            HARDWARE_GRAPHICS_COMMAND__PUTIMAGE) {
                            if (rerender_prev_hardware_frame == 0 ||
                                hgc->dst_img <= 0) {
                                if ((hgc->dst_img > 0 &&
                                     first_hardware_layer_rendered == 0) ||
                                    hgc->dst_img == dst) {
                                    hardware_img_put(
                                        hgc->dst_x1, hgc->dst_y1, hgc->dst_x2,
                                        hgc->dst_y2, hgc->src_img, hgc->dst_img,
                                        hgc->src_x1, hgc->src_y1, hgc->src_x2,
                                        hgc->src_y2, hgc->use_alpha,
                                        hgc->smooth);
                                }
                            }
                        }
                        ...
                        if (hgc->command ==
                            ...
                            if (rerender_prev_hardware_frame == 0 ||
                                ...
                                if ((hgc->dst_img > 0 &&
                                    ...
                                    hardware_img_tri2d(
                                        hgc->dst_x1, hgc->dst_y1, hgc->dst_x2,
                                        hgc->dst_y2, hgc->dst_x3, hgc->dst_y3,
                                        hgc->src_img, hgc->dst_img, hgc->src_x1,
                                        hgc->src_y1, hgc->src_x2, hgc->src_y2,
                                        hgc->src_x3, hgc->src_y3,
                                        hgc->use_alpha, hgc->smooth);
                                }
                            }
                        ...
                        if (hgc->command ==
                            ...
                            if (rerender_prev_hardware_frame == 0 ||
                                ...
                                if ((hgc->dst_img > 0 &&
                                    ...
                                    hgc->dst_img == dst) {
                                    hardware_img_tri3d(
                                        hgc->dst_x1, hgc->dst_y1, hgc->dst_z1,
                                        hgc->dst_x2, hgc->dst_y2, hgc->dst_z2,
                                        hgc->dst_x3, hgc->dst_y3, hgc->dst_z3,
                                        hgc->src_img, hgc->dst_img, hgc->src_x1,
                                        hgc->src_y1, hgc->src_x2, hgc->src_y2,
                                        hgc->src_x3, hgc->src_y3,
                                        hgc->use_alpha, hgc->smooth,
                                        hgc->cull_mode, hgc->depthbuffer_mode);
                                }
                            }
                        }
                        ...
                        if (next_hardware_command_to_remove == 0)
                            next_hardware_command_to_remove =
                                command; //!!!! should be prev to this command
                        command = hgc->next_command;
                        hgc->remove = 1;
                    } else {
            ...
            if (level == 5) {
                ...
                if (environment_2d__letterbox) {
                    ...
                    if (black_texture == 0) {
                        black_texture =
                            new_hardware_img(1, 1, &black_pixel, NULL);
                    }
                    ...
                    if (environment_2d__letterbox == 1) {
                        ...
                        hardware_img_put(
                            ((float)-environment_2d__screen_x1) /
                                    environment_2d__screen_x_scale -
                                1.0f,
                            0, -1, environment_2d__screen_height - 1,
                            black_texture, 0, 0, 0, 0, 0, 0, 0);
                        hardware_img_put(
                            environment_2d__screen_width, 0,
                            (((float)-environment_2d__screen_x1) +
                             (float)environment__window_width - 1.0f) /
                                    environment_2d__screen_x_scale +
                                1.0f,
                            environment_2d__screen_height - 1, black_texture, 0,
                            0, 0, 0, 0, 0, 0);
                    } else {
                        // horizontal stripes
                        hardware_img_put(
                            ...
                            ((float)-environment_2d__screen_y1) /
                                    environment_2d__screen_y_scale -
                                1.0f,
                            environment_2d__screen_width - 1, -1, black_texture,
                            0, 0, 0, 0, 0, 0, 0);
                        hardware_img_put(
                            0, environment_2d__screen_height,
                            environment_2d__screen_width - 1,
                            (((float)-environment_2d__screen_y1) +
                             (float)environment__window_height - 1.0f) /
                                    environment_2d__screen_y_scale +
                                1.0f,
                            black_texture, 0, 0, 0, 0, 0, 0, 0);
                    }
    ...
    if (requestedKeyboardOverlayImage) {
        ...
        src_hardware_img =
            (hardware_img_struct *)list_get(hardware_img_handles, src);
        /*
        ...
        */
        hardware_img_put(0, 0, environment_2d__screen_width - 1,
                         environment_2d__screen_height - 1, src, 0, 0, 0,
                         src_hardware_img->w - 1, src_hardware_img->h - 1, 1,
                         0);
        hardware_buffer_flush();
    }

    last_rendered_hardware_display_frame_order =
        last_hardware_display_frame_order;
...
void GLUT_MouseButton_Up(int button, int x, int y) {
    ...
    mouse_message_queue_struct *queue = (mouse_message_queue_struct *)list_get(
        mouse_message_queue_handles, handle);
